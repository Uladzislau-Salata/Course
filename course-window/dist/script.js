        ...
        switch (item.nodeName) {
          ...
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              i === 0 ? state[prop] = 'Холодое' : state[prop] = 'Теплое';
              elem.forEach((box, j) => {
                box.checked = false;
                if (i == j) {
                  box.checked = true;
...
const images = () => {
  ...
  bigImage.style.maxWidth = '90vw'; // 90% от ширины окна
  bigImage.style.maxHeight = '90vh'; // 90% от высоты окна
  bigImage.style.objectFit = 'contain'; // Сохраняет пропорции
...
const modals = state => {
  ...
  // } else if (Object.keys(state).length > 4) {
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
const tabs = (hederSelector, tabSelector, contentSelector, activeClass, display = 'block') => {
  const herder = document.querySelector(hederSelector),
  ...
  hideTabContent();
  showTabContent();
  herder.addEventListener('click', e => {
